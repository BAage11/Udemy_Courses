...
class Game():
    ...
    def __init__(self, player, monster_group):
        ...
        self.next_level_sound = pygame.mixer.Sound(
            "5_Monster_Wrangler/Sounds/next_level.wav")

        # Set font
        self.font = pygame.font.Font(
            "5_Monster_Wrangler/Fonts/Abrushow.ttf", 24)

        # Set images
        blue_image = pygame.image.load(
            "5_Monster_Wrangler/Images/blue_monster.png")
        green_image = pygame.image.load(
            "5_Monster_Wrangler/Images/green_monster.png")
        purple_image = pygame.image.load(
            "5_Monster_Wrangler/Images/purple_monster.png")
        yellow_image = pygame.image.load(
            "5_Monster_Wrangler/Images/yellow_monster.png")

        # This list corresponds to the monster_type attribute
        # int 0 -> blue, 1 -> green, 2 -> purple, 3 -> yellow
    ...
    def update(self):
        ...
        # Slow down the round time to be equal to FPS (60 frames per second)
        self.frame_count += 1
    ...
    def draw(self):
        ...
        time_text = self.font.render(
            "Round Time: " + str(self.round_time), True, WHITE)
        time_rect = time_text.get_rect()
        time_rect.topright = (WINDOW_WIDTH - 10, 5)

        warp_text = self.font.render(
            "Warps: " + str(self.player.warps), True, WHITE)
        ...
        pygame.draw.rect(
            display_surface, colors[self.target_monster_type], (WINDOW_WIDTH//2 - 32, 30, 64, 64), 2)
        pygame.draw.rect(
            display_surface, colors[self.target_monster_type], (0, 100, WINDOW_WIDTH, WINDOW_HEIGHT - 200), 4)
    ...
    def start_new_round(self):
        ...
        for i in range(self.round_number):
            self.monster_group.add(Monster(random.randint(
                0, WINDOW_WIDTH - 64), random.randint(100, WINDOW_HEIGHT - 164), self.target_monster_images[0], 0))
            self.monster_group.add(Monster(random.randint(
                0, WINDOW_WIDTH - 64), random.randint(100, WINDOW_HEIGHT - 164), self.target_monster_images[1], 1))
            self.monster_group.add(Monster(random.randint(
                0, WINDOW_WIDTH - 64), random.randint(100, WINDOW_HEIGHT - 164), self.target_monster_images[2], 2))
            self.monster_group.add(Monster(random.randint(
                0, WINDOW_WIDTH - 64), random.randint(100, WINDOW_HEIGHT - 164), self.target_monster_images[3], 3))
    ...
    def choose_new_target(self):
        ...
        # List of available monsters (left)
        target_monster = random.choice(self.monster_group.sprites())

        # Gathered from Monster() class
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image = pygame.image.load(
            "5_Monster_Wrangler/Images/knight.png")
        ...
        self.catch_sound = pygame.mixer.Sound(
            "5_Monster_Wrangler/Sounds/catch.wav")
        self.die_sound = pygame.mixer.Sound(
            "5_Monster_Wrangler/Sounds/die.wav")
        self.warp_sound = pygame.mixer.Sound(
            "5_Monster_Wrangler/Sounds/warp.wav")
...
# Monster object created in Game class function ('start new round')
...
while running:
    ...
    # Update and draw the game - No sprite inheritance, thereby no 'draw' (instead draw function within Game() class)
    my_game.update()
    my_game.draw()
